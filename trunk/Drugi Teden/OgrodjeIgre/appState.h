...
public:
	...
	virtual void		changeAppState(AppState *state, Config cfg) = 0;
...
public:
	...
	virtual void enter(Config cfg) = 0;
	virtual void exit() = 0;
...
protected:
	...
	void		changeAppState(AppState* state, Config cfg){m_pParent->changeAppState(state, cfg);}
...
#define DECLARE_CLIENTAPPSTATE_CLASS(T)					\
static void create(AppStateListener* parent, const Ogre::String name, RakPeerInterface*	client)	\
{									\
	T* myAppState = new T(client);					\
	myAppState->m_pParent = parent;					\
	parent->manageAppState(name, myAppState);			\
}
