...
void darkPause::enter()
{
    ...
    OgreFramework::getSingletonPtr()->m_pTrayMgr->createLabel(OgreBites::TL_TOP, "PauseLbl", "Pause Menu", 250);
...
void darkPause::createScene()
{
}
 
void darkPause::enter(Config cfg)
{
	enter();
}
...
bool darkPause::keyPressed(const OIS::KeyEvent &keyEventRef)
{
...
bool darkPause::keyReleased(const OIS::KeyEvent &keyEventRef)
{
...
bool darkPause::mouseMoved(const OIS::MouseEvent &evt)
{
    if(OgreFramework::getSingletonPtr()->m_pTrayMgr->injectMouseMove(evt)) return true;
    return true;
}

bool darkPause::mousePressed(const OIS::MouseEvent &evt, OIS::MouseButtonID id)
{
    if(OgreFramework::getSingletonPtr()->m_pTrayMgr->injectMouseDown(evt, id)) return true;
    return true;
}

bool darkPause::mouseReleased(const OIS::MouseEvent &evt, OIS::MouseButtonID id)
{
    if(OgreFramework::getSingletonPtr()->m_pTrayMgr->injectMouseUp(evt, id)) return true;
    return true;
}

void darkPause::update(double timeSinceLastFrame)
{
...
void darkPause::buttonHit(OgreBites::Button *button)
{
    if(button->getName() == "ExitBtn")
    {
        ...
        popAllAndPushAppState(findByName("mainMenuState"));
}

void darkPause::yesNoDialogClosed(const Ogre::DisplayString& question, bool yesHit)
{
    if(yesHit == true)
        shutdown();
    else
